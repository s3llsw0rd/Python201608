...
#the film title, description, and release year.
...
SELECT customer.email, customer.first_name, customer.last_name, address.address
FROM customer
...
#rate, and special feature. Hint: You may use LIKE function in getting the 'behind the scenes' part.

SELECT film.title, film.description, film.release_year, film.rating, film.special_features
FROM film
LEFT JOIN film_actor ON film.film_id = film_actor.film_id
WHERE film.rating = 'G'
AND film.special_features LIKE '%Behind the scenes%' AND film_actor.actor_id = 15;
